...
template <typename Symbol> typename AUTOQ::Automata<Symbol>::Symbol parse_symbol(const std::string &str) {
   std::set<std::string> vars;
   return parse_symbol<Symbol>(str, vars);
}

template <typename Symbol> AUTOQ::Automata<Symbol> parse_timbuk(const std::string &str) {
   ...
   for (const std::string &line : lines) {
      ...
      if (!are_transitions) {
         ...
         if ("Transitions" == first_word) {
            ...
            if (!str.empty()) {
               THROW_AUTOQ_ERROR("Line \"" + line + "\" has an unexpected string.");
            }
         }
            ...
            if ("States" != str_states) {
               THROW_AUTOQ_ERROR("Line \"" + line + "\" contains an unexpected string.");
            }
         ...
         std::string invalid_trans_str = AUTOQ_LOG_PREFIX + "Invalid transition \"" + line + "\".";
         ...
         if (std::string::npos == parens_begin_pos) { // no tuple of states
            if ((std::string::npos != parens_end_pos) ||
                (!std::is_same_v<Symbol, AUTOQ::PredicateAutomata::Symbol> &&
                 AUTOQ::String::contains_whitespace(lhs)) ||
                lhs.empty()) {
               THROW_AUTOQ_ERROR(invalid_trans_str);
            }
            ...
            if constexpr (std::is_same_v<Symbol, AUTOQ::TreeAutomata::Symbol>) {
               ...
               result.transitions[temp][t].insert(
                   std::vector<AUTOQ::TreeAutomata::State>()); //.stateNum.TranslateFwd(rhs));
            }
            else if constexpr (std::is_same_v<Symbol, AUTOQ::PredicateAutomata::Symbol>) {
               auto temp = parse_symbol<Symbol>(lhs);
               result.transitions[temp][t].insert(
               ...
               result.transitions[temp][t].insert(
                   std::vector<AUTOQ::SymbolicAutomata::State>()); //.stateNum.TranslateFwd(rhs));
            }
            /*******************************************************************************************************************/
         }
         else { // contains a tuple of states
            if ((std::string::npos == parens_end_pos) || (parens_begin_pos > parens_end_pos) ||
            ...
            std::string str_state_tuple = lhs.substr(parens_begin_pos + 1, parens_end_pos - parens_begin_pos - 1);
         ...
         if (std::string::npos == parens_begin_pos) { // no tuple of states -> leaf
            ...
            if constexpr (std::is_same_v<Symbol, AUTOQ::TreeAutomata::Symbol>) {
               try {
                  if (colored) {
                     ...
                     result.transitions[AUTOQ::TreeAutomata::SymbolTag(sym, AUTOQ::TreeAutomata::Tag(color))][t].insert(
                         std::vector<AUTOQ::TreeAutomata::State>());
                  }
                  else {
                     result
                         .transitions[AUTOQ::TreeAutomata::SymbolTag(Symbol(boost::lexical_cast<int>(lhs)),
                                                                     AUTOQ::TreeAutomata::Tag(1))][t]
                         .insert(std::vector<AUTOQ::TreeAutomata::State>());
                  }
               }
               catch (...) {
                  if (colored) {
                     ...
                     result
                         .transitions[AUTOQ::TreeAutomata::SymbolTag(Symbol(it->second), AUTOQ::TreeAutomata::Tag(1))]
                                     [t]
                         .insert(std::vector<AUTOQ::TreeAutomata::State>());
                  }
               }
               ...
               try {
                  if (colored) {
                     ...
                     result
                         .transitions[AUTOQ::PredicateAutomata::SymbolTag(sym, AUTOQ::PredicateAutomata::Tag(color))][t]
                         .insert(std::vector<AUTOQ::PredicateAutomata::State>());
                  }
                  else {
                     result
                         .transitions[AUTOQ::PredicateAutomata::SymbolTag(Symbol(boost::lexical_cast<int>(lhs)),
                                                                          AUTOQ::PredicateAutomata::Tag(1))][t]
                         .insert(std::vector<AUTOQ::TreeAutomata::State>());
                  }
               }
               catch (...) {
                  if (colored) {
                     ...
                     result
                         .transitions[AUTOQ::PredicateAutomata::SymbolTag(Symbol(it->second.c_str()),
                                                                          AUTOQ::PredicateAutomata::Tag(1))][t]
                         .insert(std::vector<AUTOQ::TreeAutomata::State>());
                  }
               }
               ...
               if (colored) {
                  ...
                  result.transitions[AUTOQ::SymbolicAutomata::SymbolTag(sym, AUTOQ::SymbolicAutomata::Tag(color))][t]
                      .insert(std::vector<AUTOQ::SymbolicAutomata::State>());
                  for (const auto &var : scp.getNewVars())
                  ...
                  result
                      .transitions[AUTOQ::SymbolicAutomata::SymbolTag(Symbol(scp.getSymbolicComplex()),
                                                                      AUTOQ::SymbolicAutomata::Tag(1))][t]
                      .insert(std::vector<AUTOQ::SymbolicAutomata::State>());
                  for (const auto &var : scp.getNewVars())
                     result.vars.insert(var);
               }
            ...
            if constexpr (std::is_same_v<Symbol, AUTOQ::TreeAutomata::Symbol>) {
               if (colored) {
                  ...
                  result
                      .transitions[AUTOQ::TreeAutomata::SymbolTag(Symbol(boost::lexical_cast<int>(symbol)),
                                                                  AUTOQ::TreeAutomata::Tag(1))][t]
                      .insert(state_vector);
               }
               ...
               if (colored) {
                  ...
                  std::getline(ss, token, ',');
                  auto color = boost::lexical_cast<AUTOQ::PredicateAutomata::Tag>(token);
                  result.transitions[AUTOQ::PredicateAutomata::SymbolTag(sym, AUTOQ::PredicateAutomata::Tag(color))][t]
                      .insert(state_vector);
                  ...
                  result
                      .transitions[AUTOQ::PredicateAutomata::SymbolTag(parse_symbol<Symbol>(symbol),
                                                                       AUTOQ::PredicateAutomata::Tag(1))][t]
                      .insert(state_vector);
               }
            }
               ...
               if (colored) {
                  ...
                  result.transitions[AUTOQ::SymbolicAutomata::SymbolTag(temp, AUTOQ::SymbolicAutomata::Tag(1))][t]
                      .insert(state_vector);
...
AUTOQ::Automata<Symbol> AUTOQ::Parsing::TimbukParser<Symbol, Symbol2>::parse_extended_dirac_from_istream(
    std::istream *is, bool &do_not_throw_term_undefined_error,
    const std::map<std::string, AUTOQ::Complex::Complex> &constants, const std::string &predicateConstraints) {
   bool                    start_transitions = false;
   ...
   typename EvaluationVisitor<Symbol>::segment2split_t segment2split =
       std::any_cast<typename EvaluationVisitor<Symbol>::segment2split_t>(
           visitor.let_visitor_parse_string(extended_dirac2));
   ...
   auto segment2perm     = std::any_cast<typename EvaluationVisitor<Symbol>::segment2perm_t>(
       visitor.let_visitor_parse_string(extended_dirac3));
   ...
   auto final =
       std::any_cast<std::vector<AUTOQ::Automata<Symbol>>>(visitor.let_visitor_parse_string(afterrewrite)).at(0);
...
AUTOQ::Parsing::TimbukParser<Symbol, Symbol2>::parse_n_extended_diracs_from_istream(
    std::vector<std::istream *> isVec, const std::vector<std::map<std::string, AUTOQ::Complex::Complex>> &constantsVec,
   ...
   for (std::istream *is : isVec) {
      ...
      while (std::getline(*is, line)) {
         ...
         if (line.empty()) {
            ...
            if (std::regex_search(line, std::regex("Extended +Dirac"))) {
               ...
               THROW_AUTOQ_ERROR(
                   "The section \"Extended Dirac\" should be declared first before specifying the states.");
            }
         }
      ...
      exprs[i]     = std::any_cast<std::vector<std::string>>(
          visitor.let_visitor_parse_string(extended_dirac2)); // now we assume it does NOT contain SETMINUS
      /**/
      i++;
   }
   ...
   for (size_t i = 1; i < exprs.size(); i++) {
      if (exprs[i - 1].size() != exprs[i].size()) {
         std::cout << "[ERROR] The 1st list of expressions: " << AUTOQ::Util::Convert::ToString(exprs[i - 1])
                   << std::endl;
   ...
   for (size_t i = 0; i < exprs[0].size(); ++i) {
      ...
      EvaluationVisitor<Symbol, Symbol2> visitor(
          constantsVec, predicateConstraintsVec); // any pair of constants and predicates can do
      visitor.mode = EvaluationVisitor<Symbol, Symbol2>::COLLECT_KETS_AND_COMPUTE_UNIT_DECOMPOSITION_INDICES;
      typename EvaluationVisitor<Symbol, Symbol2>::segment2split_t segment2split =
          std::any_cast<typename EvaluationVisitor<Symbol, Symbol2>::segment2split_t>(
              visitor.let_visitor_parse_string(new_composited_expression));
      ...
      auto segment2perm = std::any_cast<typename EvaluationVisitor<Symbol, Symbol2>::segment2perm_t>(
          visitor.let_visitor_parse_string(extended_diracC));
      ...
      while (std::getline(ss, item, ';')) {
         ...
         if (!trimmed.empty()) {
            ...
            auto final =
                std::any_cast<std::vector<AUTOQ::Automata<Symbol>>>(visitor.let_visitor_parse_string(afterrewrite))
                    .at(0);
            if (i == 0) {
               // resultA1 = final;
...
template <typename Symbol>
AUTOQ::Automata<Symbol> parse_extended_dirac(const std::string &str, const std::map<std::string, Complex> &constants,
                                             const std::string &predicateConstraints,
                                             bool              &do_not_throw_term_undefined_error) {
   std::istringstream inputStream(str); // delimited by '\n'
   auto               aut = AUTOQ::Parsing::TimbukParser<Symbol>::parse_extended_dirac_from_istream(
       &inputStream, do_not_throw_term_undefined_error, constants, predicateConstraints);
   // aut.print(str);
   return aut;
}
...
template <typename Symbol, typename Symbol2>
std::pair<std::vector<AUTOQ::Automata<Symbol>>, std::vector<int>>
parse_n_extended_diracs(const std::vector<std::string>                    &strVec,
                        const std::vector<std::map<std::string, Complex>> &constantsVec,
                        const std::vector<std::string>                    &predicateConstraintsVec) {
   ...
   return AUTOQ::Parsing::TimbukParser<Symbol, Symbol2>::parse_n_extended_diracs_from_istream(istreamVec, constantsVec,
                                                                                              predicateConstraintsVec);
}
...
template <typename Symbol, typename Symbol2>
AUTOQ::Automata<Symbol>
AUTOQ::Parsing::TimbukParser<Symbol, Symbol2>::ReadAutomaton(const std::string &filepath,
                                                             bool              &do_not_throw_term_undefined_error) {
   try {
      AUTOQ::Automata<Symbol>                        result;
      ...
      if (!boost::algorithm::ends_with(filepath, ".aut") &&
        (fileContents.find("Constants") != std::string::npos/* ||
         ...
         size_t found2 =
             std::min({fileContents.find("Extended"), fileContents.find("Root"), fileContents.find("Variable")});
         ...
         if (fileContents.find("Constants") != std::string::npos) {
            std::string constants_str = AUTOQ::String::trim(
                fileContents.substr(std::string("Constants").length(), found2 - std::string("Constants").length()));
         ...
         if (max_k == INT_MIN)
            max_k = 0; // IMPORTANT: if not modified, resume to 0.
...
template <typename Symbol, typename Symbol2>
std::pair<std::vector<AUTOQ::Automata<Symbol>>, std::vector<int>>
AUTOQ::Parsing::TimbukParser<Symbol, Symbol2>::ReadTwoAutomata(const std::string &filepath1,
   ...
   try {
      ...
      for (size_t i = 0; i < filepathVec.size(); i++) { // there are two automata possibly along with loop invariants
         ...
         if (!boost::algorithm::ends_with(filepathVec[i], ".aut") &&
            ...
            size_t found = std::min(
                {fileContents[i].find("Extended"), fileContents[i].find("Root"), fileContents[i].find("Variable")});
            if (found == std::string::npos) {
               THROW_AUTOQ_ERROR("Neither \"Extended Dirac\" nor \"Root States\" are specified.");
            }
            if (fileContents[i].find("Constants") != std::string::npos) {
               std::string constants_str = AUTOQ::String::trim(fileContents[i].substr(
                   std::string("Constants").length(), found - std::string("Constants").length()));
...
std::variant<AUTOQ::Automata<AUTOQ::Symbol::Concrete>, AUTOQ::Automata<AUTOQ::Symbol::Symbolic>,
             AUTOQ::Automata<AUTOQ::Symbol::Predicate>>
ReadAutomaton(const std::string &filepath) {
   std::string fileContents = AUTOQ::Util::ReadFile(filepath);
   if (fileContents.find("Predicates") != std::string::npos) {
      ...
      auto aut                               = AUTOQ::Parsing::TimbukParser<AUTOQ::Symbol::Concrete>::ReadAutomaton(
          filepath, do_not_throw_term_undefined_error);
      if (do_not_throw_term_undefined_error)
         return aut;
      else
...
std::variant<AUTOQ::Automata<AUTOQ::Symbol::Concrete>, AUTOQ::Automata<AUTOQ::Symbol::Predicate>>
ReadPossiblyPredicateAutomaton(const std::string &filepath) {
   std::string fileContents                      = AUTOQ::Util::ReadFile(filepath);
   bool        do_not_throw_term_undefined_error = true;
   auto        aut = AUTOQ::Parsing::TimbukParser<AUTOQ::Symbol::Concrete>::ReadAutomaton(filepath,
