      ...
      for (auto candidate : candidates) {
         ...
         for (const auto &top : vertex) {
            ...
            for (const auto &[c, fi] : qcfi[top]) {
               if (c & candidate) {
                  if (fi.size() != 1)
                     THROW_AUTOQ_ERROR("The sets of choices under the top state " + std::to_string(top) +
                                       "are not disjoint!");
                  ...
                  if (!is_internal.has_value()) {
                     ...
                     THROW_AUTOQ_ERROR("The selected transitions under this choice" + std::to_string(candidate) +
                                       "cannot be balanced!");
                  }
                  if (is_internal.value()) { // internal transitions
                     if (fi.begin()->second.size() != 1)
                        THROW_AUTOQ_ERROR("The sets of choices under the top state " + std::to_string(top) +
                                          "are not disjoint!");
         ...
         if (!is_internal.has_value())
            THROW_AUTOQ_ERROR("should have picked some transitions");
         if (!is_internal.value())
            return false; // if all top states have picked a leaf transition, then the language is not empty.
         if (new_vertex.empty())
            THROW_AUTOQ_ERROR("Internal transitions should have some children!");
         bfs.push(new_vertex);
      END:;
      }
   }
...
const bool inclusion_debug = false;
#define INCLUSION_DEBUG(msg)                                                                                           \
   if (inclusion_debug)                                                                                                \
   AUTOQ_DEBUG(msg)
...
template <> bool AUTOQ::Automata<AUTOQ::Symbol::Index>::operator<=(const Automata<AUTOQ::Symbol::Index> &autB) const {
      ...
      do { // Construct one new vertex for each possible combination of A's transitions.
         ...
         if (color_consistent1) {
            ...
            for (const auto &cell : vertex) {
               ...
               for (const auto &kv : A_transition_combinations) { // simply loop through all qA's in this cell
                  ...
                  for (const auto &qB : cell.at(qA)) {
                     ...
                     else if (!B_transition_combinations[qB].empty() &&
                              B_transition_combinations[qB].begin()->first != desired_symbol) {
               ...
               while (
                   !have_listed_all_combinationsB) { // Construct one new cell for each possible combination of B's transitions.
                  ...
                  if (color_consistent2 && color_consistenAB) {
                     for (const auto &kv : A_transition_combinations) {
                        ...
                        for (const auto &qB : cell.at(qA)) {
                           const auto &inB =
                               B_transition_combinations[qB].begin()->second->second; // the current input vector for qB
            ...
            if (is_leaf_vertex) { // only when considering some particular transitions
               AUTOQ_DEBUG("THE VERTEX: "
                           << AUTOQ::Util::Convert::ToString(vertex2)
                           << " LEADS A TO NOTHING OF STATES, SO WE SHALL NOT PUSH THIS VERTEX BUT CHECK IF B HAS "
                              "POSSIBLE SIMULTANEOUS TRANSITION COMBINATIONS LEADING TO THIS VERTEX.");
               if (vertex_fail) {
                  auto stop_include = std::chrono::steady_clock::now();
                  include_status    = AUTOQ::Util::Convert::toString(stop_include - start_include) + " X";
                  AUTOQ_DEBUG(
                      "UNFORTUNATELY B HAS NO POSSIBLE TRANSITION COMBINATIONS, SO THE INCLUSION DOES NOT HOLD :(");
               ...
               bfs.push(vertex2);
               AUTOQ_DEBUG("BUILD EDGE: " << AUTOQ::Util::Convert::ToString(vertex) << " -> "
                                          << AUTOQ::Util::Convert::ToString(vertex2));
            }
         }
   ...
   AUTOQ_DEBUG("FORTUNATELY FOR EACH (MAXIMAL) PATH B HAS POSSIBLE SIMULTANEOUS TRANSITION COMBINATIONS, SO THE "
               "INCLUSION DOES HOLD :)");
   total_include_time += stop_include - start_include;
   return true;
}

template <typename Symbol> bool AUTOQ::Automata<Symbol>::operator<=(const Automata<Symbol> &autB) const {
   ...
   for (const auto &t : this->transitions) {
      ...
      for (; i <= symbol_map.size(); i++) {
         ...
         if (i == symbol_map.size() || eq) {
            Automata<AUTOQ::Symbol::Index>::SymbolTag symbol_tag2 = {AUTOQ::Symbol::Index(symbol.is_leaf(), i),
                                                                     symbol_tag.tag()};
            for (const auto &out_ins : t.second) {
               const auto &out = out_ins.first;
               const auto &ins = out_ins.second;
...
bool AUTOQ::check_validity(Constraint C, const PredicateAutomata::Symbol &ps, const SymbolicAutomata::Symbol &te) {
   ...
   for (
       const auto &kv :
       regToExpr) // example: z3 <(echo '(declare-fun x () Int)(declare-fun z () Int)(assert (= z (+ x 3)))(check-sat)')
   ...
   if (result == "unsat\n")
      ...
      std::cout << str << "\n";
      std::cout << result << "-\n";
      THROW_AUTOQ_ERROR("The solver Z3 did not correctly return SAT or UNSAT.\nIt's probably because the specification "
                        "automaton is NOT a predicate automaton.");
   }
}
...
bool scaled_inclusion_with_or_without_renaming(AUTOQ::SymbolicAutomata autA, AUTOQ::SymbolicAutomata autB,
   ...
   std::vector<std::map<AUTOQ::SymbolicAutomata::SymbolTag, AUTOQ::SymbolicAutomata::StateVector>> transA(
       autA.stateNum);
   std::vector<std::map<AUTOQ::SymbolicAutomata::Symbol,
                        std::map<AUTOQ::SymbolicAutomata::Tag, AUTOQ::SymbolicAutomata::StateVector>>>
       transB(autB.stateNum);
   for (const auto &t : autA.transitions) {
      const auto &symbol_tag = t.first;
      for (const auto &out_ins : t.second) {
         const auto &out = out_ins.first;
   ...
   while (!bfs.empty()) {
      ...
      bfs.pop();
      // List all possible transition combinations of A in this vertex first!
      std::map<AUTOQ::SymbolicAutomata::State,
               typename std::map<AUTOQ::SymbolicAutomata::SymbolTag, AUTOQ::SymbolicAutomata::StateVector>::iterator>
          A_transition_combinations;
      std::map<AUTOQ::SymbolicAutomata::State, std::vector<AUTOQ::SymbolicAutomata::Tag>>
          possible_colors_for_qA; // Elements may repeat in the vector!
      for (const auto &qA_qBs : *(vertex.begin())) {
         auto qA                       = qA_qBs.first;
         A_transition_combinations[qA] = transA[qA].begin();
         for (const auto &fc_in : transA[qA]) {
      ...
      do { // Construct one new vertex for each possible combination of A's transitions.
         ...
         if (color_consistent) {
            Vertex vertex2;
            bool   vertex_fail = true; // is_leaf_vertex
            std::set<std::set<std::pair<AUTOQ::Symbol::Symbolic, AUTOQ::Symbol::Symbolic>>>
                leaf_pairs_of_one_new_vertex;
            for (const auto &cell : vertex) {
               ...
               // The following loop is used to build all possible transition combinations of B,
               // given the cell (set) of constraints, each of which describes "some A's state <==> some B's states".
               std::map<AUTOQ::SymbolicAutomata::State, std::set<AUTOQ::Symbol::Symbolic>>
                   As_symbols_associated_with_Bs_states;
               std::map<
                   AUTOQ::SymbolicAutomata::State,
                   std::map<AUTOQ::SymbolicAutomata::Symbol,
                            std::map<AUTOQ::SymbolicAutomata::Tag, AUTOQ::SymbolicAutomata::StateVector>::iterator>>
                                                                                         B_transition_combinations_data;
               ...
               for (const auto &kv : A_transition_combinations) { // simply loop through all qA's in this cell
                  ...
                  for (const auto &qB : cell.at(qA)) {
                     ...
                     if (desired_symbol.is_internal()) {
                        ...
                        else if (!B_transition_combinations_data[qB].empty() &&
                                 B_transition_combinations_data[qB].begin()->first != desired_symbol2) {
                           // If qB is enforced to take two different symbols together,
                           // simply construct the unique cell without B's states!
                           have_listed_all_combinationsB = true;
                           // Do NOT use break; here to avoid interrupting
               ...
               while (
                  ...
                  for (const auto &kv : B_transition_combinations) { // Print the current combination
                     INCLUSION_DEBUG(AUTOQ::Util::Convert::ToString(kv.second) + "[" +
                                     AUTOQ::Util::Convert::ToString(
                                         B_transition_combinations_data.at(kv.first).at(kv.second)->first) +
                                     "]" +
                                     AUTOQ::Util::Convert::ToString(
                                         B_transition_combinations_data.at(kv.first).at(kv.second)->second) +
                                     " -> " + AUTOQ::Util::Convert::ToString(kv.first));
                     all_used_colors &= B_transition_combinations_data.at(kv.first).at(kv.second)->first;
                  ...
                  if (color_consistent2) {
                     ...
                     for (const auto &kv : A_transition_combinations) {
                        ...
                        const auto &inA = kv.second->second; // the current input vector for qA
                        for (const auto &qB : cell.at(qA)) {
                           const auto &inB = B_transition_combinations_data.at(qB)
                                                 .at(B_transition_combinations.at(qB))
                                                 ->second;   // the current input vector for qB
                           assert(inA.size() == inB.size()); // one function symbol has only one arity.
                           for (unsigned i = 0; i < inA.size(); i++) {
                              cell2[inA.at(i)].insert(inB.at(i));
                           }
                  ...
                  for (auto it = B_transition_combinations.rbegin(); it != B_transition_combinations.rend(); it++) {
                     ...
                     if (std::next(B_transition_combinations_data.at(q).at(f), 1) != transB[q][f].end()) {
                        ...
                        if (it2 != B_transition_combinations_data.at(q).end()) {
                           ...
                           it->second = B_transition_combinations_data.at(q).begin()->first;
                           if (it ==
                               std::prev(B_transition_combinations.rend(), 1)) { // position equivalent to .begin()
                              have_listed_all_combinationsB = true;
                              break; // All combinations have been generated
                           }
                        }
            ...
            if (is_leaf_vertex) { // only when considering some particular transitions
               INCLUSION_DEBUG("THE VERTEX: "
                               << AUTOQ::Util::Convert::ToString(vertex2)
                               << " LEADS A TO NOTHING OF STATES, SO WE SHALL NOT PUSH THIS VERTEX BUT CHECK IF B HAS "
                                  "POSSIBLE SIMULTANEOUS TRANSITION COMBINATIONS LEADING TO THIS VERTEX.");
               if (vertex_fail) {
                  auto stop_include = std::chrono::steady_clock::now();
                  AUTOQ::SymbolicAutomata::include_status =
                      AUTOQ::Util::Convert::toString(stop_include - start_include) + " X";
                  INCLUSION_DEBUG(
                      "UNFORTUNATELY B HAS NO POSSIBLE TRANSITION COMBINATIONS, SO THE INCLUSION DOES NOT HOLD :(");
                  AUTOQ::SymbolicAutomata::total_include_time += stop_include - start_include;
                  return false;
               ...
               for (const auto &var :
                    Z) // (forall ((x1 σ1) (x2 σ2) ··· (xn σn)) (exists ((x1 σ1) (x2 σ2) ··· (xn σn)) ϕ))
                  assertion += "(" + var + " Real)";
               ...
               for (const auto &leaf_pairs_of_one_new_cell : leaf_pairs_of_one_new_vertex) {
                  assertion += "(or ";
                  for (const auto &pair : leaf_pairs_of_one_new_cell) {
                     define_varA += "(define-fun E" + std::to_string(counter) + "r () Real " +
                                    pair.first.complex.realToSMT() + "); " +
                                    (std::ostringstream{} << pair.first).str() + "\n";
                     define_varA += "(define-fun E" + std::to_string(counter) + "i () Real " +
                                    pair.first.complex.imagToSMT() + "); " +
                                    (std::ostringstream{} << pair.first).str() + "\n";
                     ...
                     define_varA += "(define-fun F" + std::to_string(counter) + "r (" + arguments + ") Real " +
                                    pair.second.complex.realToSMT() + "); " +
                                    (std::ostringstream{} << pair.second).str() + "\n";
                     define_varA += "(define-fun F" + std::to_string(counter) + "i (" + arguments + ") Real " +
                                    pair.second.complex.imagToSMT() + "); " +
                                    (std::ostringstream{} << pair.second).str() + "\n";
                     assertion += "(distinct ";
                     ...
                     if (arguments.empty())
                        assertion += "(- (* ratioR F" + std::to_string(counter) + "r) (* ratioI F" +
                                     std::to_string(counter) + "i)))\n";
                     else
                        assertion += "(- (* ratioR (F" + std::to_string(counter) + "r" + arguments + ")) (* ratioI (F" +
                                     std::to_string(counter) + "i" + arguments + "))))\n";
                     assertion += "(distinct ";
                     assertion += "E" + std::to_string(counter) + "i ";
                     if (arguments.empty())
                        assertion += "(+ (* ratioR F" + std::to_string(counter) + "i) (* ratioI F" +
                                     std::to_string(counter) + "r)))\n";
                     else
                        assertion += "(+ (* ratioR (F" + std::to_string(counter) + "i" + arguments + ")) (* ratioI (F" +
                                     std::to_string(counter) + "r" + arguments + "))))\n";
                     counter++;
                  }
                  assertion += ")\n"; // "(or "
               }
               ...
               INCLUSION_DEBUG("THE VERTEX: " << AUTOQ::Util::Convert::ToString(vertex2)
                                              << " HAS BEEN CREATED BEFORE.");
            }
            else {
               created.insert(vertex2);
               bfs.push(vertex2);
               INCLUSION_DEBUG("BUILD EDGE: " << AUTOQ::Util::Convert::ToString(vertex) << " -> "
                                              << AUTOQ::Util::Convert::ToString(vertex2));
            }
         }
   ...
   auto stop_include                       = std::chrono::steady_clock::now();
   AUTOQ::SymbolicAutomata::include_status = AUTOQ::Util::Convert::toString(stop_include - start_include);
   INCLUSION_DEBUG("FORTUNATELY FOR EACH (MAXIMAL) PATH B HAS POSSIBLE SIMULTANEOUS TRANSITION COMBINATIONS, SO THE "
                   "INCLUSION DOES HOLD :)");
   AUTOQ::SymbolicAutomata::total_include_time += stop_include - start_include;
   return true;
}

template <> bool AUTOQ::SymbolicAutomata::operator<<=(AUTOQ::SymbolicAutomata autB) const {
   return scaled_inclusion_with_or_without_renaming(*this, autB, false);
}

template <> bool AUTOQ::SymbolicAutomata::operator_scaled_inclusion_with_renaming(AUTOQ::SymbolicAutomata autB) const {
...
template <> bool AUTOQ::TreeAutomata::operator<<=(AUTOQ::TreeAutomata autB) const {
   ...
   std::vector<
       std::map<AUTOQ::TreeAutomata::Symbol, std::map<AUTOQ::TreeAutomata::Tag, AUTOQ::TreeAutomata::StateVector>>>
       transB(autB.stateNum);
   for (const auto &t : autA.transitions) {
      const auto &symbol_tag = t.first;
      for (const auto &out_ins : t.second) {
   ...
   while (!bfs.empty()) {
      ...
      bfs.pop();
      // List all possible transition combinations of A in this vertex first!
      std::map<AUTOQ::TreeAutomata::State,
               typename std::map<AUTOQ::TreeAutomata::SymbolTag, AUTOQ::TreeAutomata::StateVector>::iterator>
          A_transition_combinations;
      std::map<AUTOQ::TreeAutomata::State, std::vector<AUTOQ::TreeAutomata::Tag>>
          possible_colors_for_qA; // Elements may repeat in the vector!
      for (const auto &qA_qBs : *(vertex.begin())) {
         auto qA                       = qA_qBs.first;
         A_transition_combinations[qA] = transA[qA].begin();
      ...
      do { // Construct one new vertex for each possible combination of A's transitions.
         ...
         if (color_consistent) {
            ...
            for (const auto &cell : vertex) {
               ...
               // The following loop is used to build all possible transition combinations of B,
               // given the cell (set) of constraints, each of which describes "some A's state <==> some B's states".
               std::map<AUTOQ::TreeAutomata::State, std::set<AUTOQ::Symbol::Concrete>>
                   As_symbols_associated_with_Bs_states;
               std::map<AUTOQ::TreeAutomata::State,
                        std::map<AUTOQ::TreeAutomata::Symbol,
                                 std::map<AUTOQ::TreeAutomata::Tag, AUTOQ::TreeAutomata::StateVector>::iterator>>
                                                                                 B_transition_combinations_data;
                  ...
                  for (const auto &pair : leaf_pairs) {
                     for (int i = 0; i < 4; i++) {
                        if (pair.first.complex.isZero() && pair.second.complex.isZero())
                           ...
                           else if (!(ratio.first * pair.second.complex)
                                         .valueEqual(ratio.second * pair.first.complex)) {
                              color_consistent2 = false;
                              goto L1372;
                           }
                        }
            ...
            if (is_leaf_vertex) { // only when considering some particular transitions
               ...
               if (vertex_fail) {
                  auto stop_include = std::chrono::steady_clock::now();
                  AUTOQ::TreeAutomata::include_status =
                      AUTOQ::Util::Convert::toString(stop_include - start_include) + " X";
                  INCLUSION_DEBUG(
                      "UNFORTUNATELY B HAS NO POSSIBLE TRANSITION COMBINATIONS, SO THE INCLUSION DOES NOT HOLD :(");
...
template <> bool AUTOQ::TreeAutomata::operator_scaled_inclusion_with_renaming(AUTOQ::TreeAutomata autB) const {
...
template <> bool AUTOQ::PredicateAutomata::operator<<=(AUTOQ::PredicateAutomata) const {
   THROW_AUTOQ_ERROR("PredicateAutomata::operator<<= not implemented");
}

template <> bool AUTOQ::IndexAutomata::operator<<=(AUTOQ::IndexAutomata) const {
   THROW_AUTOQ_ERROR("IndexAutomata::operator<<= not implemented");
}

template <> bool AUTOQ::ConstrainedAutomata::operator<<=(AUTOQ::ConstrainedAutomata) const {
